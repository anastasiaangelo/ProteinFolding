...
# def clean_json_data(results):
#     # Flatten the nested 'ground_state_info' dictionaries
...
def generate_exact_energies_data(res_list, rot_list):
    """
    Generate a DataFrame with energy data for different residue and rotamer configurations.
    """
    data = []  # Store rows as dictionaries for efficiency

    for num_res, num_rot in product(res_list, rot_list):
        energies, bitstrings = get_all_states_efficient(num_res, num_rot)
        data.append({'num_res': num_res, 'num_rot': num_rot, 'energies': energies, 'bitstrings': bitstrings})
...
def find_min_energy_and_bitstring_from_exact_energy_dataframe(df_exact, nres, nrot):
    ...
    # Get the energy and its corresponding bitstring from the row
    energy_data = df_filtered.iloc[0]['energies']

    # Check if the data is a string and needs to be evaluated
    if isinstance(energy_data, str):
        energy = ast.literal_eval(energy_data)
    else:
        energy = energy_data
    bitstring_data = df_filtered.iloc[0]['bitstrings']

    # Check if the data is a string and needs to be evaluated
    if isinstance(bitstring_data, str):
        corresponding_bitstring = ast.literal_eval(bitstring_data)
    else:
        corresponding_bitstring = bitstring_data
...
def find_number_of_shots_to_find_ground_state(df, df_exact, **kwargs):
    ...
    # Get exact ground state values
    min_energy, corresponding_bitstring = find_min_energy_and_bitstring_from_exact_energy_dataframe(
        df_exact, kwargs['num_res'], kwargs['num_rot']
    )
    ...
    for nshots in df_filtered['shots'].unique():
        ...
        intermediate_data = df2.iloc[0]['intermediate_data']

        if isinstance(intermediate_data, str):
            intermediate_data = ast.literal_eval(intermediate_data)

        index = next(
            (i + 1 for i, data in enumerate(intermediate_data) if any(
                int_to_bitstring(int(key), total_bits) == corresponding_bitstring for key in data.keys()
            )),
            None
        )
...
def generate_minimum_shots_to_ground_state_data(df, df_exact):
    param_combinations = list(product(
        df['num_res'].unique(), df['num_rot'].unique(),
        df['alpha'].unique(), df['p'].unique()
    ))

    results = []

    for num_res, num_rot, alpha, p in param_combinations:
        num_iter, shots, num_shots = find_number_of_shots_to_find_ground_state(
            df, df_exact, num_res=num_res, num_rot=num_rot, alpha=alpha, p=p
        )

        num_qubits = num_res * num_rot

        results.append([num_res, num_rot, num_qubits, alpha, p, num_iter, shots, num_shots])

    df_min_iter = pd.DataFrame(results, columns=['num_res', 'num_rot', 'num_qubits', 'alpha', 'p', 'min_iter', 'shots', 'min_shots'])

    # Sort by relevant parameters
    df_min_iter = df_min_iter.sort_values(
        by=['num_res', 'num_rot', 'num_qubits', 'alpha', 'min_shots', 'p', 'shots'], 
...
def compute_mean_shots_to_ground_state(df, df_exact):
    from itertools import product
    ...
    for num_res, num_rot, alpha, p, shots in param_combinations:
        df_filtered = df[
            (df['num_res'] == num_res) &
            (df['num_rot'] == num_rot) &
            (df['alpha'] == alpha) &
        ...
        total_bits = num_res * num_rot
        min_energy, ground_state = find_min_energy_and_bitstring_from_exact_energy_dataframe(
            df_exact, num_res, num_rot
        )

        iter_counts = []

        for _, row in df_filtered.iterrows():
            intermediate_data = row['intermediate_data']
            if isinstance(intermediate_data, str):
                intermediate_data = ast.literal_eval(intermediate_data)

            index = next(
                (i + 1 for i, data in enumerate(intermediate_data) if any(
                    int_to_bitstring(int(key), total_bits) == ground_state for key in data.keys()
                )),
                None
            )

            if index is not None:
                iter_counts.append(index)

        if iter_counts:
            mean_iter = sum(iter_counts) / len(iter_counts)
            mean_total_shots = mean_iter * shots
            results.append([num_res, num_rot, alpha, p, shots, mean_iter, mean_total_shots])

    df_mean_shots = pd.DataFrame(results, columns=[
        'num_res', 'num_rot', 'alpha', 'p', 'shots', 'mean_iter', 'mean_shots'
    ])

    return df_mean_shots.sort_values(by=['num_res', 'num_rot', 'alpha', 'p', 'shots'])



# def generate_minimum_shots_to_ground_state_data(df, df_exact, df_min_shots=None):
#     param_combinations = list(product(
#         df['num_res'].unique(), df['num_rot'].unique(),
#         df['alpha'].unique(), df['p'].unique()
#     ))
...
#     if df_min_shots is not None:
#         df_min_shots = df_min_shots.copy()
#         df_min_shots['p'] = df_min_shots['p'].astype(float).round(6)
#         df_min_shots['min_iter'] = df_min_shots['min_iter'].astype(float).round(6)
#         df_min_shots['shots'] = df_min_shots['shots'].astype(float).round(6)
#         existing_records = set(
#             zip(df_min_shots['num_res'], df_min_shots['num_rot'],
#                 df_min_shots['alpha'].round(6), df_min_shots['p'], df_min_shots['shots'])
#         )

#     for num_res, num_rot, alpha, p in param_combinations:
#         p = round(float(p), 6)
#         print(f"\nProcessing num_res={num_res}, num_rot={num_rot}, alpha={alpha}, p={p}")
#         try:
#             filtered_df = df[(df['num_res'] == num_res) & (df['num_rot'] == num_rot) &
#                              (df['alpha'].round(6) == round(alpha, 6)) &
#                              (df['p'].round(6) == p)]
            
#             if filtered_df.empty:
#                 print(f"Warning: No relevant data for num_res={num_res}, num_rot={num_rot}, alpha={alpha}, p={p}. Skipping...")
#                 continue
            
#             num_iter, shots, num_shots = find_number_of_shots_to_find_ground_state(
#                 filtered_df, df_exact, num_res=num_res, num_rot=num_rot, alpha=alpha, p=p
#             )

#             if num_iter is None or shots is None or num_shots is None:
#                 print(f"Warning: No results found for num_res={num_res}, num_rot={num_rot}. Skipping...")
#                 continue
            
#             num_qubits = num_res * num_rot
#             num_iter, shots, num_shots = round(float(num_iter), 6), round(float(shots), 6), round(float(num_shots), 6)
            
#             if (num_res, num_rot, round(alpha, 6), p, shots) in existing_records:
#                 print(f"Skipping existing entry: num_res={num_res}, num_rot={num_rot}, alpha={alpha}, p={p}, shots={shots}")
#                 continue

#             results.append([num_res, num_rot, num_qubits, alpha, p, num_iter, shots, num_shots])

#         except Exception as e:
#             print(f"Error processing num_res={num_res}, num_rot={num_rot}: {e}. Skipping...")

#     df_new = pd.DataFrame(results, columns=['num_res', 'num_rot', 'num_qubits', 'alpha', 'p', 'min_iter', 'shots', 'min_shots'])
    
#     if df_min_shots is not None:
#         df_min_iter = pd.concat([df_min_shots, df_new], ignore_index=True)
#     else:
#         df_min_iter = df_new
    
#     df_min_iter = df_min_iter.sort_values(
#         by=['num_qubits', 'p', 'shots', 'num_res', 'num_rot', 'alpha', 'min_shots'], 
#         ascending=[True, True, True, True, True, True, True]
#     )
...
def plot_heatmap(df_min_iter):
    pivot_table = df_min_iter.pivot_table(index='p', columns='shots', values='min_shots', aggfunc='min')

    plt.figure(figsize=(10, 6))
    sns.heatmap(pivot_table, annot=True, cmap="viridis", fmt=".0f", linewidths=0.5)
    plt.xlabel("Shots")
    plt.ylabel("p")
    plt.show()
...
def find_final_parameters(df):
    """
    Extract the final (optimal) set of parameters for each row in the DataFrame.

    """

    df_new = df[['num_res', 'num_rot', 'alpha', 'shots', 'p']].copy()
    ...
    df_new['parameters'] = df['parameters'].apply(ast.literal_eval)

    def extract_final_params(param_list):
        return param_list[-1] if isinstance(param_list, list) and param_list else None

    df_new['optimal_parameters'] = df_new['parameters'].apply(extract_final_params)
