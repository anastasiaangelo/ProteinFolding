...
class Simulation:
    ...
    def statevector_simulation_parallel(self):
        if len(self.hyperparameters) == 5: 
            num_rot, num_res, shots, alpha, p = self.hyperparameters
            SIMULATION_ID = None 
        else:
            num_rot, num_res, shots, alpha, p, SIMULATION_ID = self.hyperparameters
        log_info(f"Running statevector_simulation_parallel with num_rot: {num_rot}, num_res: {num_res}, shots: {shots}, alpha: {alpha}, p: {p}, SIMULATION_ID: {SIMULATION_ID}")
    ...
    def statevector_simulation_parallel_trained(self):
        ...
        SP.statevector_simulation_XY_parallel_trained(num_rot=num_rot, num_res=num_res, shots=shots, alpha=alpha, p=p)
...
if __name__ == "__main__":
    ...
    simulation_name = "statevector_simulation_parallel"
