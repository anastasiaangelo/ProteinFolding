...
for TASK_ID in $(seq 1 $NUM_TASKS); do
    ...
    cp "$HOMEDIR/simulations_array_trained.py" "$TASK_RUNDIR/"
    ...
    jbsub -q x86_24h \
          ...
          "cd $TASK_RUNDIR && source /u/aag/proteinfolding/ccc_venv/bin/activate && python3 -u simulations_array_trained.py \"$COMMIT\" \"$TASK_ID\""
