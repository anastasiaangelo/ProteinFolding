#!/bin/bash
#SBATCH --job-name=ProteinFolding
#SBATCH --array=1-64%2         # 54 tasks, 50 running in parallel
...
cp "$HOMEDIR/simulations_array_job.py" "$TASK_RUNDIR/"

# Move to the task-specific directory
cd "$TASK_RUNDIR" || exit

# Activate environment
source /home/b/aag/proteinfolding/venv/bin/activate
...
python3 -u simulations_array_job.py "$COMMIT" "$SLURM_ARRAY_TASK_ID" \
